...
from book_library_app.utils import get_schema_args, apply_order, apply_filter, get_pagination, token_required, calculate_stats, validate_json_content_type, calculate_stats
...
def create_vote(user_id: int, args: dict):
    ...
    if Votes.query.filter(Votes.user_id == user_id, Votes.book_id == args['book_id']).first():
        abort(409, description=(
            f'User already add comment on this book'))
