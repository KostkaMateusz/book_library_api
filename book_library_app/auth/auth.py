...
def register(args: dict):
    if User.query.filter(User.username == args["username"]).first():
        abort(
            409, description=(f'User with username { args["username"] } alredy exists')
        )

    if User.query.filter(User.email == args["email"]).first():
        abort(409, description=(f'User with email { args["email"] } alredy exists'))
...
def reset_password(args: dict, hash: str):
    ...
    hash_record = HashResetTable.query.filter(HashResetTable.hash_code == hash).first()
    if hash_record is None:
        abort(404, "Invalid adres site")

    user = User.query.get_or_404(hash_record.user_id, description="Wrong adres")
